    ...
    <table class="table table-striped table-hover table-bordered">
        ...
        <tbody>
            <tr>
                <td>Total</td>
                <td
                class="{total >= 0 ? 'text-success' : 'text-danger'}">{cost(total)}</td>
            </tr>
            <tr>
                <td>Debit</td>
                <td
                class="{debit >= 0 ? 'text-success' : 'text-danger'}"
                >{cost(debit)}</td>
            </tr>
            <tr>
                <td>Credit</td>
                <td
                    class="{credit >= 0 ? 'text-success' : 'text-danger'}"
                >{cost(credit)}</td>
            </tr>
            <tr>
                <td>Savings</td>
                <td
                class="{savings >= 0 ? 'text-success' : 'text-danger'}">{cost(savings)}</td>
            </tr>
            <tr>
                <td>Other</td>
                <td
                class="{other >= 0 ? 'text-success' : 'text-danger'}">{cost(other)}</td>
            </tr>
        </tbody>
    </table>
</div>